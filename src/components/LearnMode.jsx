...
const LearnMode = ({ prefecturesData }) => {
  // 学習済みの都道府県IDを保存
  ...
  useEffect(() => {
    ...
    const sorted = [...prefecturesData].sort((a, b) => {
  ...
  useEffect(() => {
    let filtered = prefecturesData;
    
    if (filterMode === 'high') {
      // 頻出のみ
      filtered = prefecturesData.filter(pm => pm.examFrequency === 'high');
    } else if (filterMode === 'review') {
      // 復習モード：覚えていない都道府県のみ
      filtered = prefecturesData.filter(pm => !learnedIds.includes(pm.id));
    }
    ...
    setCurrentIndex(0);
  }, [filterMode, cardOrder, prefecturesData, learnedIds]);
      ...
      <div className="bg-white rounded-xl shadow-lg p-6 mb-6 min-h-[400px]">
        ...
        <div className="flex flex-wrap gap-2 justify-center mb-6">
          ...
          <span className={`px-3 py-1 rounded-full text-sm text-white ${
            ...
            currentCard.examFrequency === 'medium' ? 'bg-yellow-500' : 'bg-primary-500'
          }`}>
        ...
        <div className="mb-4">
          <p className="text-sm text-gray-600">
            <span className="font-semibold">地方：</span>
            {currentCard.party}
          </p>
        </div>
