...
const DailyPM = ({ prefecturesData }) => {
  ...
  useEffect(() => {
    ...
    if (lastDate !== today) {
      // 新しい日なので、ランダムに都道府県を選択
      const randomIndex = Math.floor(Math.random() * prefecturesData.length);
      setDailyPM(prefecturesData[randomIndex]);
      setLastDate(today);
    } else {
      // 既に今日の都道府県が選択されている場合は、LocalStorageから復元
      // （簡易的に、日付が同じなら最初の都道府県IDを使用）
      const dayHash = today.split('-').reduce((a, b) => parseInt(a) + parseInt(b), 0);
      const index = dayHash % prefecturesData.length;
      setDailyPM(prefecturesData[index]);
    }
  }, [prefecturesData, lastDate, setLastDate]);

  // 手動で次の都道府県を選択
  const selectNextPM = () => {
    if (!dailyPM) return;
    const currentIndex = prefecturesData.findIndex(pm => pm.id === dailyPM.id);
    const nextIndex = (currentIndex + 1) % prefecturesData.length;
    setDailyPM(prefecturesData[nextIndex]);
  };
    ...
    <div className="max-w-4xl mx-auto px-4 py-6">
      ...
      <div className="card-modern p-6 mb-4 animate-fade-in">
        <div className="flex items-center gap-3 mb-6">
          <BookOpen size={28} className="text-primary-600 animate-float" />
          <h2 className="text-3xl font-bold gradient-text">今日の都道府県</h2>
        </div>
        ...
        <div className="mb-4">
          ...
          <div className="flex flex-wrap gap-2">
            ...
            <span className={`px-2 py-1 rounded-md text-xs font-semibold text-white ${
              ...
              dailyPM.examFrequency === 'medium' ? 'bg-yellow-500' : 'bg-primary-500'
            }`}>
