...
import allPrimeMinistersData from '../data/allprefectures.json';

const QuizMode = ({ prefecturesData }) => {
  // データモード選択
  ...
  // クイズ開始
  const startQuiz = () => {
    const currentData = dataMode === 'important' ? prefecturesData : allPrimeMinistersData;
  ...
  const getScoreMessage = (score) => {
    ...
    if (score >= 50) return { emoji: '👍', text: 'もう少しです！', color: 'text-primary-600' };
    ...
    return (
      <div className="max-w-4xl mx-auto px-4 py-6">
        ...
        <div className="bg-white rounded-xl shadow-lg p-6">
          ...
          <button
            onClick={startQuiz}
            className="w-full py-4 bg-primary-600 text-white rounded-lg font-bold text-lg hover:bg-primary-700 transition-colors shadow-md flex items-center justify-center gap-2"
          >
            クイズを始める
            <ArrowRight size={24} />
          </button>
      ...
      <div className="max-w-4xl mx-auto px-4 py-6">
          ...
          <div className="mb-6">
            <span className="inline-block px-3 py-1 bg-primary-100 text-primary-700 rounded-full text-sm font-semibold mb-4">
              {currentQuestion.type === 'name' ? '名前当て' :
               currentQuestion.type === 'achievement' ? '特産物当て' : '順序当て'}
            </span>
                      ...
                      {!showFeedback && (
                        <div className="flex gap-2">
                          <button
                            ...
                            disabled={index === 0}
                            className="px-3 py-1 bg-primary-100 text-primary-700 rounded disabled:opacity-30 disabled:cursor-not-allowed"
                          >
                            ↑
                          </button>
                          <button
            ...
            <div className="space-y-3">
              {currentQuestion.options.map((option, index) => {
                ...
                if (showResult) {
                  ...
                  buttonClass += 'border-gray-200 hover:border-primary-500 hover:bg-primary-50';
                }
        ...
        {showFeedback && (
          <button
            ...
            className="w-full py-4 bg-primary-600 text-white rounded-lg font-bold text-lg hover:bg-primary-700 transition-colors shadow-md"
          >
